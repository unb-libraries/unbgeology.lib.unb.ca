    ...
    <PvTableColumn v-for="[column, header] in columns" :key="column" :field="column" :header="header">
      <template #body="slotProps">
        <slot :name="column" :value="slotProps.data[column]" :entity="slotProps.data">
          {{ column in slotProps.data ? slotProps.data[column] : '' }}
        </slot>
      </template>
    </PvTableColumn>
    <PvTableColumn field="self">
      <template #body="{ data }">
        <div class="invisible flex flex-row justify-end group-hover:visible">
          <a :href="`/dashboard/${data.self.substring(5)}`" :title="`Edit ${label(data) ?? `item`}`" class="h-8 w-8 rounded-md p-1 hover:cursor-pointer hover:bg-yellow-400">
            <svg
          ...
          <a :title="`Remove ${label(data) ?? `item`}`" class="h-8 w-8 rounded-md p-1 hover:cursor-pointer hover:bg-red-500" @click.prevent="onDelete(data.self)">
            <svg
              xmlns="http://www.w3.org/2000/svg"
              fill="none"
              viewBox="0 0 24 24"
