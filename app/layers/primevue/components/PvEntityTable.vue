    ...
    <PvTableColumn field="self">
      <template #body="{ data }">
        <div class="invisible flex flex-row justify-end group-hover:visible">
          <a :href="`/dashboard/${data.self.substring(5)}`" :title="`Edit ${label(data) ?? `item`}`" class="hover:bg-yellow h-8 w-8 rounded-md p-1 hover:cursor-pointer">
            <svg
          ...
          <a :title="`Remove ${label(data) ?? `item`}`" class="hover:bg-red h-8 w-8 rounded-md p-1 hover:cursor-pointer" @click.prevent="remove(data)">
            <svg
              xmlns="http://www.w3.org/2000/svg"
              fill="none"
              viewBox="0 0 24 24"
