...
const { data: specimen, error } = await useFetch<EntityJSON<Specimen>>(props.self, {
  query: {
    select: ['id', 'name', 'type', 'classification', 'origin', 'images', 'status']
  }
})
if (error.value) {
  throw new Error(`Specimen ${props.self} not found.`)
}
</script>