<template>
  <div class="group relative size-full" :data-placeholder="!Boolean(specimen.images?.entities?.length) ? '' : undefined">
    ...
    <button v-if="(specimen.images?.entities ?? []).length > 1"
      class="absolute left-2 top-[calc((100%-4.125rem-1px)/2-1rem)] hidden group-hover:flex bg-base-17 dark:bg-accent-6 hover:bg-accent-26 dark:hover:bg-accent-36 rounded-sm shadow-lg shadow-base-17/60 dark:shadow-accent-6/60"
    ...
    <button v-if="(specimen.images?.entities ?? []).length > 1"
      class="absolute right-2 top-[calc((100%-4.125rem-1px)/2-1rem)] hidden group-hover:flex bg-base-17 dark:bg-accent-6 hover:bg-accent-26 dark:hover:bg-accent-36 rounded-sm shadow-lg shadow-base-17/60 dark:shadow-accent-6/60"
