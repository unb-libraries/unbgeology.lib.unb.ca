...
export function getEntityQueryParams<T extends Entity = Entity>(event: H3Event, props: string[]) {
  const { filter, page, pageSize, select, search, sort, ...where } = getQuery<EntityQuery<T>>(event)
  ...
  const parse = ([field, param]: [string, FilterParam | FilterParam[]]): Partial<Filter<Indexed<T>>> => {
    const [base, ...rest] = field.split(`.`)
    return (rest.length
      ? { [base]: parse([rest.join(`.`), param]) }
      : { [base]: Array.isArray(param) ? param.map(match).reduce((merged, single) => defu(merged, single), {}) : match(param) }) as Partial<Filter<Indexed<T>>>
  }
  ...
  return {
    ...
    select: (Array.isArray(select) ? select : select ? [select] : props)
      .filter(field => props.includes(String(field))),
    sort: (Array.isArray(sort) ? sort : sort ? [sort] : [])
      .filter(field => props.includes(field.replace(/^-/, ``)))
      .map(field => field.startsWith(`-`) ? [field.slice(1), -1] : [field, 1]) as [keyof T, 1 | -1][],
...
export function getFilter(field: string, operator: FilterOperator) {
