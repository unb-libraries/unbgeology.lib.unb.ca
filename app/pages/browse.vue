<template>
  <div class="flex flex-col space-y-8 h-full">
    ...
    <ul class="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-6 xl:grid-cols-8 2xl:grid-cols-10 gap-2 overflow-y-scroll">
      <li v-for="classification in list?.entities" class="cursor-pointer hover:border-accent-dark hover:text-accent-dark dark:hover:border-accent-mid dark:hover:text-accent-mid aspect-square flex justify-center items-center bg-primary-20 dark:bg-primary-80 border border-primary-40 dark:border-primary-60"><a href="/specimens">{{ classification.label }}</a>
      </li>
    </ul>
  </div>
</template>
